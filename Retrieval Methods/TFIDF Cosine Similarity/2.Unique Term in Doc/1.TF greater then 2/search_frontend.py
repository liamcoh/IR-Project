...
def search():
    ...
    # TFIDF Cos sim calc
    q_vec = generate_query_tfidf_vector(tokens, index_body_tf_sample)
    tfidf = generate_document_tfidf_matrix(tokens, index_body_tf_sample)
    ...
    if len(res_score) < 100:
        tfidf = generate_document_tfidf_matrix_top100(tokens, index_body_tf_sample, res_score)
        cos_sim = cosine_similarity(tfidf, q_vec)
