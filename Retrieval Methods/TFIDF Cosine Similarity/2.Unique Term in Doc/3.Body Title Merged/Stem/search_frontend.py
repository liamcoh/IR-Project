...
with open("postings_gcp_title_stem_sample/index.pkl", 'rb') as f:
    index_title_stem_sample = pickle.load(f)
...
def search():
    ...
    tokenstem = [stemmer.stem(t) for t in tokens if t not in all_stopwords]

    # TFIDF Cos sim calc
    q_vec = generate_query_tfidf_vector(tokenstem, index_body_stem_sample)
    tfidf = generate_document_tfidf_matrix(tokenstem, index_body_stem_sample, "postings_gcp_body_stem_sample/")
    cos_sim = cosine_similarity(tfidf, q_vec)

    # Sort Documents by Cos sim score
    res_score1 = sorted([(doc_id, score) for doc_id, score in cos_sim.items()], key=lambda x: x[1],reverse=True)

    # Get Title score by unique tokens in title
    dict_results = {}
    for token in np.unique(tokenstem):
        if token not in index_title_stem_sample.df.keys():
            continue
        # Get posting list for token
        posting = read(index_title_stem_sample, token, "postings_gcp_title_stem_sample/")
    ...
    res_score2 = sorted([(doc_id, score / max_title) for doc_id, score in dict_results.items() if doc_id in
                         dict(res_score1).keys()], key=lambda x: x[1], reverse=True)

    res_score = merge_results(res_score1, res_score2, w1=0.25, w2=0.75, N=100)

    # Single token query handler and fill for low intersection document
    if len(res_score) < 100:
        q_vec = generate_query_tfidf_vector(tokenstem, index_body_stem_sample)
        tfidf = generate_document_tfidf_matrix_top100(tokenstem, index_body_stem_sample,
                                                      res_score, "postings_gcp_body_stem_sample/")
        cos_sim = cosine_similarity(tfidf, q_vec)
...
def search_body():
    ...
    # TFIDF Cos sim calc
    q_vec = generate_query_tfidf_vector(tokens, index_body_stem_sample, "postings_gcp_body_stem_sample/")
    tfidf = generate_document_tfidf_matrix(tokens, index_body_stem_sample, "postings_gcp_body_stem_sample/")
    cos_sim = cosine_similarity(tfidf, q_vec)
...
def search_title():
    ...
    for token in np.unique(tokens):
        if token not in index_title_sample.df.keys():
            continue
        # Get posting list for token
        posting = read(index_title_sample, token, "postings_gcp_title_stem_sample/")
        for doc_id, tf in posting:
            if doc_id in dict_results:
                dict_results[doc_id] = dict_results[doc_id] + 1
            else:
