...
with open("postings_gcp_body_bm25_stem_sample/index.pkl", 'rb') as f:
    index_body_bm25_stem_sample = pickle.load(f)

with open("postings_gcp_2gram_body_sample/index.pkl", 'rb') as f:
    index_2gram_body_sample = pickle.load(f)
...
def search():
    ...
    for term in np.unique(tokens):
        if term in index_title_stem_sample.df.keys():
            tmp = read(index_body_bm25_stem_sample, term, "postings_gcp_body_bm25_stem_sample/")
            prec = np.percentile([bm for doc_id, bm in tmp], 50)
            tmp = {doc_id: bm for doc_id, bm in tmp if bm > prec}
            plsbody[term] = tmp
            plstitle[term] = read(index_title_stem_sample, term, "postings_gcp_title_stem_sample/")

    for term in np.unique(ngrams_tokens):
        if term in index_2gram_body_sample.df.keys():
            pls[term] = dict(read(index_2gram_body_sample, term, "postings_gcp_2gram_body_sample/"))
    ...
    if len(ngrams_tokens) > 0 and pls_size > 0:
        candidates = get_candidate_documents(ngrams_tokens, index_2gram_body_sample, pls)
        res_score1 = sorted([(doc_id, bm25_score_calc(ngrams_tokens, doc_id, index_2gram_body_sample, pls, 2.1, 0.1)/1000)
        ...
        if len(res_score1) < 100:
            ...
            res_score1 += sorted([(doc_id, bm25_score(tokens, doc_id, index_body_bm25_stem_sample, plsbody) / 1000)
    ...
    if len(res_score1) < 100:
        candidates = get_candidate_documents(tokens, index_body_bm25_stem_sample, plsbody)
        res_score1 += sorted([(doc_id, bm25_score(tokens, doc_id, index_body_bm25_stem_sample, plsbody) / 1000)
                              for doc_id in candidates], key=lambda x: x[1], reverse=True)[
                      :100 - len(res_score1)]
    ...
    res_score = merge_results_pr(res_score1, res_score2, pr_dict, w1=0.5, w2=0.2, w3=0.3, N=100)
