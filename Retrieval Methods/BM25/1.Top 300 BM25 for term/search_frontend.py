...
def search():
    ...
    for term in np.unique(tokens):
        if term in index_body_bm25_stem_sample.df.keys():
            pls[term] = dict(read(index_body_bm25_stem_sample, term, "postings_gcp_body_bm25_stem_sample/")[-300:])

    candidates = get_candidate_documents(tokens, index_body_bm25_stem_sample, pls)
    res_score = sorted([(doc_id, bm25_score(tokens, doc_id, index_body_bm25_stem_sample, pls))
                        for doc_id in candidates], key=lambda x: x[1], reverse=True)[:100]
